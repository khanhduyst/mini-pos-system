...
<?php foreach ($categories as $cat): ?>
<?php
        $is_edit_error = (isset($_SESSION['error_edit_cat_id']) && $_SESSION['error_edit_cat_id'] == $cat['id']);
        ?>
        ...
        <div class="modal-content border-0 shadow rounded-3">
            ...
            <form action="/category/edit" method="POST" novalidate>
                ...
                <div class="modal-body p-4 bg-white">
                    <div class="row g-3">
                        <div class="col-12"><label class="form-label small fw-semibold text-secondary">Mã danh
                                mục</label><input type="text" class="form-control bg-light" name="category_code"
                                value="<?php echo $cat['category_code']; ?>" readonly></div>
                        <div class="col-12">
                            ...
                            <input type="text" class="form-control <?php echo $is_edit_error ? 'is-invalid' : ''; ?>"
                                name="category_name" value="<?php echo $cat['category_name']; ?>" required>
                            <?php if ($is_edit_error): ?>
                            <div class="invalid-feedback fw-semibold small mt-1 d-block">
                                <?php echo $_SESSION['error_edit_cat_msg']; ?></div>
                            <?php endif; ?>
                        </div>
                        <div class="col-12"><label class="form-label small fw-semibold text-secondary">Mô tả
                                ngắn</label><textarea class="form-control" name="description"
                                rows="3"><?php echo $cat['description']; ?></textarea></div>
                    </div>
                </div>
    ...
    <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content border-0 shadow-lg rounded-3">
            <div class="modal-body p-4 text-center bg-white rounded-3">
                ...
                <h5 class="fw-bold text-dark mb-2">Tạm ẩn danh mục?</h5>
                <p class="text-secondary small mb-4">Hệ thống sẽ tạm ẩn nhóm <strong
                        class="text-dark"><?php echo $cat['category_name']; ?></strong> khỏi bộ lọc màn hình bán hàng.
                </p>
                <?php else: ?>
                <div class="text-success mb-3"><i class="bi bi-eye-fill fs-1"></i></div>
                <h5 class="fw-bold text-dark mb-2">Kinh doanh lại?</h5>
                <p class="text-secondary small mb-4">Kích hoạt hiển thị lại danh mục nhóm hàng <strong
                        class="text-dark"><?php echo $cat['category_name']; ?></strong>.</p>
                <?php endif; ?>
                <div class="d-flex gap-2 justify-content-center">
                    <button type="button" class="btn btn-light fw-semibold rounded-2 px-3 small"
                        data-bs-dismiss="modal">Hủy</button>
                    <a href="/category/toggle?id=<?php echo $cat['id']; ?>&status=<?php echo $cat['status']; ?>"
                        class="btn fw-semibold rounded-2 px-4 small text-white <?php echo $cat['status'] == 1 ? 'btn-danger' : 'btn-success'; ?>">Xác
                        nhận</a>
                </div>
            </div>
        </div>
...
<?php
$is_add_error = isset($_SESSION['error_add_cat_name']);
$old_add = $is_add_error ? $_SESSION['old_add_cat_data'] : [];
?>
                    ...
                    <div class="row g-3">
                        <div class="col-12">
                            ...
                            <div class="input-group">
                                <input type="text" class="form-control rounded-start-2 shadow-none"
                                    id="add_category_code" name="category_code" placeholder="DM01"
                                    value="<?php echo isset($old_add['category_code']) ? htmlspecialchars($old_add['category_code']) : ''; ?>"
                                    required>
                        ...
                        <div class="col-12">
                            ...
                            <input type="text" class="form-control <?php echo $is_add_error ? 'is-invalid' : ''; ?>"
                                name="category_name" placeholder="Ví dụ: Nước ngọt, Bánh kẹo..."
                                value="<?php echo isset($old_add['category_name']) ? htmlspecialchars($old_add['category_name']) : ''; ?>"
                                required>
                            <?php if ($is_add_error): ?>
                            <div class="invalid-feedback fw-semibold small mt-1 d-block">
                                <?php echo $_SESSION['error_add_cat_name']; ?></div>
                            <?php endif; ?>
                        </div>
                        <div class="col-12"><label class="form-label small fw-semibold text-secondary">Mô tả đặc điểm
                                nhóm hàng</label><textarea class="form-control" name="description" rows="3"
                                placeholder="Ghi chú thêm..."><?php echo isset($old_add['description']) ? htmlspecialchars($old_add['description']) : ''; ?></textarea>
                        </div>
                    </div>
                </div>
                <div class="modal-footer border-top p-3 bg-white">
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const editForms = document.querySelectorAll('form[action="/category/edit"]');
    ...
    <?php if (isset($_SESSION['error_add_cat_name'])): ?>
    const addModal = new bootstrap.Modal(document.getElementById('addCategoryModal'));
    addModal.show();
    <?php unset($_SESSION['error_add_cat_name']);
            unset($_SESSION['old_add_cat_data']); ?>
    <?php endif; ?>

    <?php if (isset($_SESSION['error_edit_cat_id'])): ?>
    const editModalId = 'editCategoryModal' + '<?php echo $_SESSION['error_edit_cat_id']; ?>';
    const editModal = new bootstrap.Modal(document.getElementById(editModalId));
    editModal.show();
    <?php unset($_SESSION['error_edit_cat_id']);
            unset($_SESSION['error_edit_cat_msg']); ?>
    <?php endif; ?>
});
</script>
