...
class ProductController
{
    ...
    private function uploadToCloudinary($file_tmp, $product_code)
    {
        ...
        if ($err) {
            die("Lỗi kết nối cURL: " . $err);
        }

        $result = json_decode($response, true);

        if (isset($result['error'])) {
            die("Lỗi từ phía Cloudinary: " . $result['error']['message']);
        }
    ...
    public function add()
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if ($this->productModel->isCodeOrBarcodeExists($product_code, $variants)) {
                $_SESSION['flash_error'] = "Lỗi: Mã hàng gốc hoặc mã vạch biến thể đã tồn tại!";
    ...
    public function edit()
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if ($this->productModel->isCodeOrBarcodeExists($product_code, $variants, $id)) {
                $_SESSION['flash_error'] = "Lỗi: Thông tin cập nhật dính trùng lặp mã vạch hàng hóa khác!";
                header("Location: /product/index");
                exit();
            }
